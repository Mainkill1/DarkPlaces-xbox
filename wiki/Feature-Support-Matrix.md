...
| Generic textured mesh | Bring-up fallback: the classic candidate currently uses DarkPlaces' immediate mesh-submission diagnostic instead of pbGL client arrays. The array path displayed vertical green corruption; the immediate path produced readable loading art. This preserves the same engine vertices, indices, colors and texture coordinates while isolating submission; world rendering is not yet validated. | GL error trace + immediate-versus-array reference captures + map render |
| World/material texture resolution by memory profile | Deliberate bounded-resolution fallback: canonical staging creates one content-addressed stored override PK3 per profile. `stock64` caps general TGA assets at 256 px and external Q3 lightmaps at 64 px; `dev128` uses 512/128 px. The Xbox loader streams lightmap decode/conversion/upload one image at a time; resulting GPU lightmaps do not use world picmip. `retail64`/`xemu64` also enforce `gl_picmip >= 2`; `dev128` preserves the saved runtime choice. The disc/runtime profile is checked before autoplay. Originals remain immutable. | deterministic converter/loader fixtures + profile-specific content identity + controlled visual comparison + physical high-water evidence |
| Q3 BSP portal culling | Deliberate stock-memory fallback: the classic Xbox loader uses the Q3 BSP's authored PVS and node/leaf bounds with the engine's existing frustum-culling path. It does not reconstruct DarkPlaces' optional runtime portal graph; Q1 behavior remains unchanged. Portal-dependent SVBSP light culling is also bypassed when the graph is absent. | generated-source contracts + boot trace + map/realtime-light render comparison |
...
| `glArrayElement` driver-test path | Disabled; the normal array path uses pbGL `glDrawElements`, while the current canonical bring-up candidate selects DarkPlaces' separate immediate feeder. Forcing `gl_mesh_testarrayelement` remains a deliberate no-op. | link audit + native mesh capture |
| 2D console/menu | Required; client-array submission produced vertical green corruption, while the immediate fallback produced readable loading art. Menu interaction and complete gameplay presentation remain unvalidated. | text and UI screenshots |
